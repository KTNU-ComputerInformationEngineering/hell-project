...
router.get('/write', function(req,res,next){
  pool.getConnection(function(err, connection)
{
  var sqlForSelectList = 'SELECT * from res';
  connection.query(sqlForSelectList, function(err, rows){
    if(err) console.error("err : " + err);
      res.render('write',{title : "게시판 글 쓰기", rows:rows});
  })
...
router.post('/write', function(req,res,next){
    ...
    var datas = [creator_id,title,content,passwd,resname];

    pool.getConnection(function (err, connection)
    {
        // Use the connection
        var sqlForInsertBoard = "insert into board(creator_id, title, content, passwd, resname) values(?,?,?,?,?)";
